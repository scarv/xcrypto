...
\XCRYPTOINSTR{xc.bop}{rd, rs1, rs2, l}{}{
  ...
  $f \ASN \FIELD{\SPR{ubop}}{\ID{BOP}_{\VERB[RV]{l}}}$ \;
  $t \ASN 0$ \;
...
\XCRYPTOINSTR{xc.xsll.i}{rd, rs1, rs2, cshamt}{}{
  Compute an XOR, after preparing the second operand via a  left-shift;
  one could view this as a specific version of operations possible via
  the so-called ARM ``flexible second operand'' in associated ISAs.
}{
  $\GPR[*][{\VERB[RV]{rd}}] \ASN \GPR[*][{\VERB[RV]{rs1}}] \XOR ( \GPR[*][{\VERB[RV]{rs2}}] \OP[w][u]{\LSH} \VERB[RV]{cshamt} )$ \;
}

% ----------------------------------------------------------------------------

\XCRYPTOINSTR{xc.xsrl.i}{rd, rs1, rs2, cshamt}{}{
  Compute an XOR, after preparing the second operand via a right-shift;
  one could view this as a specific version of operations possible via
  the so-called ARM ``flexible second operand'' in associated ISAs.
}{
  $\GPR[*][{\VERB[RV]{rd}}] \ASN \GPR[*][{\VERB[RV]{rs1}}] \XOR ( \GPR[*][{\VERB[RV]{rs2}}] \OP[w][u]{\RSH} \VERB[RV]{cshamt} )$ \;
}

% ----------------------------------------------------------------------------

\XCRYPTOINSTR{xc.xrol.i}{rd, rs1, rs2, cshamt}{}{
  Compute an XOR, after preparing the second operand via a  left-rotate;
  one could view this as a specific version of operations possible via
  the so-called ARM ``flexible second operand'' in associated ISAs.
}{
  $\GPR[*][{\VERB[RV]{rd}}] \ASN \GPR[*][{\VERB[RV]{rs1}}] \XOR ( \GPR[*][{\VERB[RV]{rs2}}] \OP[w][u]{\LRT} \VERB[RV]{cshamt} )$ \;
}

% ----------------------------------------------------------------------------

\XCRYPTOINSTR{xc.xror.i}{rd, rs1, rs2, cshamt}{}{
  Compute an XOR, after preparing the second operand via a right-rotate;
  one could view this as a specific version of operations possible via
  the so-called ARM ``flexible second operand'' in associated ISAs.
}{
  $\GPR[*][{\VERB[RV]{rd}}] \ASN \GPR[*][{\VERB[RV]{rs1}}] \XOR ( \GPR[*][{\VERB[RV]{rs2}}] \OP[w][u]{\RRT} \VERB[RV]{cshamt} )$ \;
}
