...
destination registers. The intermediate result $t$ is a $34$
bit integer, comprising the unsigned sum of {\tt rs1}, {\tt rs2}. and
{\tt rs3}. 

The low $32$ bits of $t$ are written to {\tt rd1}, while the upper 2 bits
are written to {\tt rd2}.

\begin{itemize}
\item \INST{ADD.mp (rd2, rd1), rs1, rs2, rs3     }{
...
\begin{itemize}
\item \INST{SUB.mp (rd2, rd1), rs1, rs2, rs3     }{
...
The \ASM{ACC} instruction concatenates the two destination 32-bit registers
into a single 64-bit value. It then adds the value in {\tt rs3}, and stores
the result back.

\begin{itemize}
\item \INST{ACC.mp (rd2, rd1), rs1, rs2, rs3     }{
