...
\begin{itemize}
\item \INST{CMOV.cr rd, rs1, rs2}{
    $\CPR[*][rd] \ASN (\CPR[*][rs2] == 0) ? \CPR[*][rs1] : \CPR[*][rd]$
}
\item \INST{CMOVN.cr rd, rs1, rs2}{
    $\CPR[*][rd] \ASN (\CPR[*][rs2] != 0) ? \CPR[*][rs1] : \CPR[*][rd]$
}
\end{itemize}
...
\begin{itemize}
\item \INST{RSEED.cr rs1}{
    $RNG \ASN \CPR[*][rs1]$
}
\item \INST{RSAMP.cr rd}{
    $\CPR[*][rd] \ASN RNG$
}
\end{itemize}
...
\subsection{Packed Field Manipulation}


\instdesc{TWID.B, TWID.Hx, TWID.Cx}
