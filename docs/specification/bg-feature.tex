...
precise specification, keep in mind that
support for a given feature class can be determined programmatically using 
an associated CSR,
and
all feature classes in \XCID can emulated by using RV32I alone.
...
A suite of instructions 
(e.g., \VERB[RV]{xc.ld.w} and \VERB[RV]{xc.st.w})
that allow 
transfer of content between
   memory                         (i.e., $\MEM$)
and
...
that allow 
manipulation, e.g.,
 sampling entropy from (via \VERB[RV]{xc.rngsamp})
or
injecting entropy into (via \VERB[RV]{xc.rngseed})
...
In particular, 
a suite of instructions 
(e.g., \VERB[RV]{xc.scatter.b} and \VERB[RV]{xc.gather.b})
that allow 
...
encryption (e.g., of network traffic), and a rich associated literature
(see, e.g., \REFAPPX{appx:related}); put simply, ISEs designed to support 
AES can be a sane and well explored approach.

\XCID includes a suite of instructions 
(e.g., \VERB[RV]{xc.aes.sub} and \VERB[RV]{xc.aes.mix})
drawn directly from the design and analysis of 
