...
\XCINSTR{xc.bop}{crd, crs1, crs2, lut4}{
  ...
  $
  \INDEX{r}{i} = f( \INDEX{x}{i}, \INDEX{y}{i}, \INDEX{z}{i} ) = 
     \INDEX{\VERB[RV]{lut4}}{x_i \CONS y_i \CONS z_i}
  $
  ...
  Any 1, 2 or 3 input boolean function can be represented by appropriately
  padding the \VERB[RV]{lut8} immediate.
  \[
  ...
  \end{array}
  \]
  These examples show how the standard 2-input RISC-V boolean functions
  can be synthesised.
}{
  \For{$i = 0$ {\bf upto} $31$} {
...
\VERB[RV]{ca} \CONS \VERB[RV]{cb} \CONS \VERB[RV]{cc} &=& \RADIX{100}{2} &\mapsto& \VERB[RV]{n} &=&  1 & w &=& 32 {\bf *} \\
...
{\bf *} Note that the \RADIX{100}{2} encoding is valid only for the instructions
\ASM{xc.psll.i},
\ASM{xc.psrl.i} and
\ASM{xc.prot.i}.
In this case, the \VERB[RV]{ca} bit is used to encode the most significant
bit of the shift/rotate amount.
For instructions other than these, a pack width encoding of \RADIX{100}{2}
