...
\XCINSTR{xc.ld.bu}{crd, h, b, imm11(rs1)}{
  ...
  (cf. \VERB[RV]{xc.ld.b}, which sets them to zero).
}{
...
\XCINSTR{xc.ld.hu}{crd, h,    imm11(rs1)}{
  ...
  (cf. \VERB[RV]{xc.ld.h}, which sets it   to zero).
}{
  $addr                                                                    \ASN \GPR[*][{\VERB[RV]{rs1}}] + \EXT[32]{\pm}( \VERB[RV]{imm11} )$ \;
...
\XCINSTR{xc.ld.b} {crd, h, b, imm11(rs1)}{
  \iencldbu
}{
  Load a     byte from memory
  into a specific     byte of a $\XCR$ register,
  setting all other     bytes in said register to zero
  (cf. \VERB[RV]{xc.ld.bu}, which leaves them un-modified).
}{
  $addr                                          \ASN \GPR[*][{\VERB[RV]{rs1}}] + \EXT[32]{\pm}( \VERB[RV]{imm11} )$ \;
  $index_0                                       \ASN ( \VERB[RV]{cc} \CONS \VERB[RV]{cd} ) \XOR 0$ \;
...
\XCINSTR{xc.ld.h} {crd, h,    imm11(rs1)}{
  \iencldhu
}{
  Load a halfword from memory
  into a specific halfword of a $\XCR$ register,
  setting the other halfword  in said register to zero
  (cf. \VERB[RV]{xc.ld.hu}, which leaves it   un-modified).
}{
  $addr                                          \ASN \GPR[*][{\VERB[RV]{rs1}}] + \EXT[32]{\pm}( \VERB[RV]{imm11} )$ \;
  $index_0                                       \ASN ( \VERB[RV]{cc} \CONS             0 ) \XOR 0$ \;
  $index_1                                       \ASN ( \VERB[RV]{cc} \CONS             0 ) \XOR 1$ \;
...
\XCINSTR{xc.st.b} {rs2, h, b, imm11(rs1)}{
  ...
  $addr            \ASN \GPR[*][{\VERB[RV]{rs1}}] + \EXT[32]{\pm}( \VERB[RV]{imm11} )$ \;
  $index_0         \ASN ( \VERB[RV]{cc} \CONS \VERB[RV]{cd} ) \XOR 0$ \;
  $\MEM[*][addr  ] \ASN \INDEX[8]{\XCR[*][{\VERB[RV]{crd}}]}{index_0}$ \;
}
...
\XCINSTR{xc.pclmul.l}{pw, crd, crs1, crs2}{
  ...
  Compute a   register-based packed multiplication in $\F_2[\IND]$ (or ``carry-less'' multiplication),
...
\XCINSTR{xc.mclmul.1}{(crd2, crd1), crs1, crs2, crs3}{
  ...
  Compute a   register-based multi-precision (or ``long'') multiply-accumulate in $\F_2[\IND]$:
  the $32$-bit multiplicands
  $\XCR[*][{\VERB[RV]{crs1}}]$
  and
  $\XCR[*][{\VERB[RV]{crs2}}]$
...
\subsubsection{Class-$3.1$: AES encryption}
\label{sec:spec:instr:3:1}

Per~\cite[Figures 5+6]{SCARV:TilGro:06}, the idea would be to support

\begin{enumerate}
\item \INST{aes.sub rd0, rd1, rs0, rs1, rs2, rs3}{
        $\GPR[*][\OPER{rd0}] \ASN \SCOPE{AES}{\ALG{S-box}}( \GPR[*][\OPER{rs1}]_{15,\ldots,12} ) \CONS
                                  \SCOPE{AES}{\ALG{S-box}}( \GPR[*][\OPER{rs0}]_{11,\ldots, 8} ) \CONS
                                  \SCOPE{AES}{\ALG{S-box}}( \GPR[*][\OPER{rs1}]_{ 7,\ldots, 4} ) \CONS
                                  \SCOPE{AES}{\ALG{S-box}}( \GPR[*][\OPER{rs0}]_{ 3,\ldots, 0} )$\;
      }
\item \INST{aes.mix rd0, rd1, rs0, rs1, rs2, rs3}{
        $\GPR[*][\OPER{rd0}] \ASN \SCOPE{AES}{\ALG{Mix-Column}}\big( \GPR[*][\OPER{rs1}]_{15,\ldots,12} ) \CONS
                                                                     \GPR[*][\OPER{rs1}]_{11,\ldots, 8} ) \CONS
                                                                     \GPR[*][\OPER{rs0}]_{ 7,\ldots, 4} ) \CONS
                                                                     \GPR[*][\OPER{rs0}]_{ 3,\ldots, 0} \big)$\;
      }
\end{enumerate}

% -----------------------------------------------------------------------------

\subsubsection{Class-$3.2$: AES decryption}
\label{sec:spec:instr:3:2}
