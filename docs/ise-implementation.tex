...
\SIGNALS{
\SIGNALI{ 1}{creg\_[a|b|c]\_ren  }{Register read port enable}
\SIGNALI{ 4}{creg\_[a|b|c]\_addr }{Register read port address}
\SIGNALO{32}{creg\_[a|b|c]\_rdata}{Register read data}
}{
...
\SIGNALS{
\SIGNALI{ 4}{creg\_d\_wen  }{Register byte write port enable}
\SIGNALI{ 4}{creg\_d\_addr }{Register write port address}
\SIGNALO{32}{creg\_d\_wdata}{Register write data}
}{
Interface signals for the register file write port.
All signals are synchronous to \SIGREF{g\_clk}.
The 4-bit \SIGREF{creg\_d\_wen} signal is a per-byte lane write enable
...
\SIGNALI{32}{palu\_imm}{
    Decoded instruction immediate. Includes the 16-bit operands for
    {\tt LLI/LUI}, shift distances and the 4-bit LUT for {\tt BOP}.
}
\SIGNALI{ 3}{palu\_pw}{
    The pack width of the operation to perform.
    The encoding of this signal corresponds to the encoding of the pack
    widths in the ISE specification document.
}
\SIGNALI{ 4}{palu\_op}{
    The operation that the ALU should perform.
}
\SIGNALO{32}{palu\_out}{The 32-bit output result.}
...
\SIGNALS{
\SIGNALI{32}{malu\_rs1}{Register operand 1}
\SIGNALI{32}{malu\_rs2}{Register operand 2}
\SIGNALI{32}{malu\_rs3}{Register operand 3}
\SIGNALI{ 4}{malu\_op}{
    The operation that the ALU should perform.
}
\SIGNALO{64}{malu\_out}{
    The 64-bit result.
    Results from the MP ALU are written back over two cycles.
    This is controlled by the FSM block, which selects either the high
    or low word to write back. It is possible to overlap writing back of
    the low word with computing the high word value in some cases.
}
